...
import io
import argparse
from utils_history import load_all_entries
...
- 【重要】对于输入文本操作，请使用通用格式描述，如"输入文本'100'"而不是"输入100到自定义最低价输入框"。只需要明确输入的文本内容，不需要指定具体的输入框。
- 【重要】以下子任务已被成功执行过：{list(finished_subtasks)}。
- 【提示】如果上一步操作失败（如误操作进入了错误页面），你可以考虑返回上一界面，但也可以根据实际情况判断是否需要继续当前任务。
- 【重要】输入文本操作必须严格按照以下顺序执行：
  1. 点击第一个输入框 → 输入对应内容 → 点击第二个输入框 → 输入对应内容
  2. 绝对不允许连续点击多个输入框后再输入内容
  3. 每次点击输入框后，下一个子任务必须是输入对应的文本内容
  4. 示例错误顺序：点击最低价输入框 → 点击最高价输入框 → 输入文本'100'
  5. 示例正确顺序：点击最低价输入框 → 输入文本'100' → 点击最高价输入框 → 输入文本'3000'
请直接输出最终的 task。
"""
